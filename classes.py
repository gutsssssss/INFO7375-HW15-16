...
class Neuron:
    ...
    def backward(self, gradient_z: torch.Tensor, inputs: torch.Tensor):
        self.gradients_w = torch.divide(torch.mm(torch.transpose(inputs, 0, 1), gradient_z), arg.batch_size)


class Layer:
    def __init__(self, num_neurons: int, num_inputs: int, activation: str, dropout: bool = False):
        self.neurons = [Neuron(num_inputs) for _ in range(num_neurons)]
        self.inputs = torch.zeros(arg.batch_size, num_inputs)
    ...
    def backward(self, gradients_a: torch.Tensor = None, y_true: torch.Tensor = None) -> torch.Tensor:
        ...
        # self.gradients_bias = gradients_z
        self.gradients_bias = torch.sum(gradients_z, dim=1)
...
# class BNLayer(Layer):
#     def __init__(self, num_inputs: int, num_neurons: int, activation: str = 'linear'):
#         super().__init__(num_neurons, num_inputs, activation)
#         self.inputs = torch.zeros(arg.batch_size, num_inputs)
#         self.input_hat = torch.zeros(arg.batch_size, num_inputs)
#         self.running_mean = torch.mean(self.inputs, dim=1, keepdim=True)
#         self.running_var = torch.var(self.inputs, dim=1, keepdim=True)
#         self.gamma = 1
#         self.beta = 0
#         self.gradient_gama = 0
#         self.gradient_beta = 0
#         self.gradient_i = torch.zeros(arg.batch_size, num_inputs)
#         self.gradient_i_hat = torch.zeros(arg.batch_size, num_inputs)
#         self.momentum = arg.momentum
#
#     def forward(self, inputs: torch.Tensor, test: bool = False):
#         self.inputs = inputs
#         self.running_mean = self.momentum*torch.mean(inputs, dim=1, keepdim=True)+(1 - self.momentum)*self.running_mean
#         self.running_var = self.momentum*torch.var(inputs, dim=1, keepdim=True)+(1 - self.momentum)*self.running_var
#         self.input_hat = torch.divide(self.inputs - self.running_mean, torch.sqrt(self.running_var) + 1e-10)
#         y = torch.mul(self.input_hat, self.gamma) + self.beta
#         return y
#
#     def backward(self, gradients_a: torch.Tensor = None, y_true: torch.Tensor = None):
#         self.gradient_gama = torch.sum(torch.mm(self.input_hat, gradients_a), dim=1)
#         self.gradient_beta = torch.sum(gradients_a, dim=1)
#         self.gradient_i_hat = torch.mul
...
class Model:
    ...
    def init_layers(self):
        # self.InitLayersInBatch([128], 'relu', batch_norm=True)
        # self.InitLayersInBatch([10], 'softmax')
        # Develop a Multilayer Neural Network @lab1
        self.InitLayersInBatch([10, 8, 8, 4], 'relu')
        self.InitLayersInBatch([1], 'sigmoid')
    ...
    def InitLayersInBatch(self, neuron_number_list: list[int], activation: str = 'relu', dropout: bool = False,
        ...
        # if batch_norm:
        #     self.layers.append(BNLayer(neuron_number_list[-1], neuron_number_list[-1], 'linear'))
...
# # Perform convolution without padding and without striding @HW13
def convolution(original_image, filter_matrix):
    output_feature_map = np.zeros((4, 4))
    for i in range(4):
        for j in range(4):
            output_feature_map[i, j] = np.sum(original_image[i:i + 3, j:j + 3] * filter_matrix)
    return output_feature_map

# depth wise convolution @HW15
def depth_wise_convolution(image, kernel):
    ...
    for i in range(channels):
        output[:, :, i] = cv2.filter2D(image[:, :, i], -1, kernel)

    return output

# point wise convolution @HW15
def point_wise_convolution(image, kernel):
...
# convolution @HW15
def conv_image(image_path, kernel, convolution_type):
    # Load the image
    image = cv2.imread(image_path)

    # Perform the specified type of convolution
    if convolution_type == 'depthwise':
        output = depth_wise_convolution(image, kernel)
    elif convolution_type == 'pointwise':
        output = point_wise_convolution(image, kernel)
    else:
